...
bool Cli::initializeMNNConvertArgs(modelConfig &modelPath, int argc, char **argv) {
    ...
    if (result.count("framework")) {
        ...
        std::cout << options.help({""}) << std::endl;
        DLOG(INFO) << "framework Invalid, use -f ONNX/MNN/JSON !";
        return false;
    }
    if (result.count("OP")) {
    ...
    // prototxt file path (unused for ONNX, kept for compatibility)
